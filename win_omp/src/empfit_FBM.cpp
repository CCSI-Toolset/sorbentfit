...
int main() {
	...
	std::string cstream("results/config_FBM_");
	...
	std::string flstream("results/filelist_FBM_"); // Creates a string to be used for the new filelist file
	...
	empfuneval fun2("config/config_FBM_empfit.txt", "filelist/filelist.txt");
	int scale_mode = fun2.scale_mode;
	int chec = 0;
	bool PSO = true;
	boost::numeric::ublas::vector<bool> logger(16);
	for (int i = 0; i != 15; i++) {
		if (fun2.override[i]) {
			chec++;
		}
