...
protected:
  ...
  bool function(boost::numeric::ublas::vector<double> const &par, boost::numeric::ublas::matrix<double> &xpass, boost::numeric::ublas::vector<double> &ypass);
	...
	while (!constream.eof() ) {
		...
		if (line[0] != '#' && line[0] != ' ' && line.length() > 0) {
			...
			if (i == 0) {
				...
				if (str.size() == 1)
					nodesize = boost::lexical_cast<double>(str[0]);
				else {error = i+1;}
			}
			i++;
		}
...
bool empfuneval::function(boost::numeric::ublas::vector<double> const &par, boost::numeric::ublas::matrix<double> &xpass, boost::numeric::ublas::vector<double> &ypass) {
	int j = 0;
	bool inbounds = true;
	for (int i = 0; i != 13; i++) {
		if (!override[i]) {
  ...
  do {
    ...
    if (ypasset) {
      ...
      *ypass_it = (((timestep+1)*delt - *(xpass_it1-1).rbegin())*(xbn[0]*M*nv + xbn[1]*(M + H) + xbn[2]*H) + (*(xpass_it1-1).rbegin() - timestep*delt)*(xb[0]*M*nv + xb[1]*(M + H) + xb[2]*H))/(wtpersite*nv*delt);
